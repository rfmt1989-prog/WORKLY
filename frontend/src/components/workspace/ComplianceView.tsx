...
type SourceRecord =
  | { kind: "document"; item: DemoDocument }
...
export function ComplianceView() {
  const { user } = useAuth();
  const { state, language, updateProject, notify } = useWorklyData();
  ...
  const canManageRequirements = Boolean(
    user?.role === "company" && user.permissions?.includes("projects.manage"),
  );
  ...
  const selectedWorker = useMemo(
    () => state?.workers.find((worker) => worker.id === selectedRow?.worker_id),
    [selectedRow?.worker_id, state?.workers],
  );
  ...
  const findSourceRecord = useCallback((issue: ComplianceIssue): SourceRecord | null => {
    ...
    if (issue.kind === "document") {
      const item = selectedWorker.documents.find(
        (document) => compactText(document.category) === compactText(issue.requirement),
      );
      return item ? { kind: "document", item } : null;
    }
    const required = compactText(issue.requirement);
    const item = selectedWorker.certificates.find((certificate) => {
      const name = compactText(certificate.name);
  ...
  const saveRequirements = async () => {
    ...
    try {
      await updateProject(selectedRow.project_id, {
        compliance_requirements: {
          documents: parseRequirementInput(documentsInput),
          certificates: parseRequirementInput(certificatesInput),
        },
      });
      await load();
      setRequirementsOpen(false);
      notify(text.requirementSaved, "success");
    } finally {
  ...
  return (
    <>
          ...
          <View style={[styles.grid, compact ? styles.gridCompact : null]}>
            ...
            {selectedRow ? <Card accent={statusColor(selectedRow.status)} style={styles.detailCard}>
              <View style={styles.detailHeader}>
                <View style={{ flex: 1 }}><Text style={styles.detailTitle}>{selectedRow.worker_name}</Text><Text style={styles.meta}>{selectedRow.worker_profession} · {selectedRow.project_name}</Text></View>
              ...
              <View style={{ gap: 8, marginTop: 12 }}>
                {selectedRow.issues.length ? selectedRow.issues.map((issue, index) => {
                  ...
                  const source = findSourceRecord(issue);
                  return (
                    <Pressable
                      key={`${issue.kind}-${issue.requirement}-${index}`}
                      disabled={!source}
                      onPress={() => source && setSourceRecord(source)}
                      style={({ pressed }) => [styles.issue, source ? styles.issueInteractive : null, pressed ? { opacity: 0.72 } : null]}
                    >
                      <Ionicons name={issue.kind === "certificate" ? "ribbon-outline" : "document-text-outline"} size={18} color={color} />
                      <View style={{ flex: 1 }}>
                        <Text style={styles.issueTitle}>{issue.label}</Text>
                        <Text style={styles.meta}>{issue.kind === "certificate" ? text.certificate : text.document} · {issueLabel(issue, text)}{issue.days_remaining !== null ? ` · ${issue.days_remaining} ${text.days}` : ""}{issue.expires_at ? ` · ${new Date(issue.expires_at).toLocaleDateString(localeForLanguage(language))}` : ""}</Text>
                      </View>
                      {source ? <View style={styles.recordLink}><Text style={[styles.recordLinkText, { color }]}>{text.openRecord}</Text><Ionicons name="chevron-forward" size={15} color={color} /></View> : null}
                    </Pressable>
                  );
              ...
              <View style={styles.requirementsHeader}>
                ...
                {canManageRequirements ? <Button compact label={text.configure} icon="options-outline" accent={accent} onPress={openRequirements} /> : null}
              </View>
              <View style={styles.requirements}>
                {selectedRow.requirements.documents.map((item) => <View key={`d-${item}`} style={styles.req}><Ionicons name="document-outline" size={14} color={workspaceColors.textSoft} /><Text style={styles.reqText}>{item}</Text></View>)}
                {selectedRow.requirements.certificates.map((item) => <View key={`c-${item}`} style={styles.req}><Ionicons name="ribbon-outline" size={14} color={workspaceColors.textSoft} /><Text style={styles.reqText}>{item}</Text></View>)}
              </View>
            </Card> : null}
          </View>
        )}
      ...
      <ModalPanel
        ...
        subtitle={selectedRow ? `${selectedRow.project_name} · ${text.configureSubtitle}` : text.configureSubtitle}
        footer={<><Button label={text.cancel} variant="ghost" onPress={() => setRequirementsOpen(false)} /><Button label={text.save} icon="checkmark" accent={accent} loading={savingRequirements} onPress={() => void saveRequirements()} /></>}
      >
        <View style={styles.editorBody}>
          <Field label={`${text.requiredDocuments} · ${text.onePerLine}`} multiline value={documentsInput} onChangeText={setDocumentsInput} />
          <Field label={`${text.requiredCertificates} · ${text.onePerLine}`} multiline value={certificatesInput} onChangeText={setCertificatesInput} />
        </View>
      </ModalPanel>

      <ModalPanel
        visible={Boolean(sourceRecord)}
        onClose={() => setSourceRecord(null)}
        title={text.sourceRecord}
        subtitle={sourceRecord?.kind === "certificate" ? text.certificate : text.document}
        footer={<Button label={text.cancel} variant="secondary" onPress={() => setSourceRecord(null)} />}
      >
        {sourceRecord ? <SourceRecordView record={sourceRecord} text={text} language={language} /> : null}
      </ModalPanel>
    </>
  );
}
...
function SourceRecordView({ record, text, language }: { record: SourceRecord; text: Copy; language: LanguageCode }) {
  ...
  const expiry = record.kind === "certificate" ? record.item.expires_at : undefined;
  const title = record.kind === "certificate" ? record.item.name : record.item.title;
  const fileName = record.item.file_name;
  return (
    <View style={styles.sourceBody}>
      <Text style={styles.sourceTitle}>{title}</Text>
      <InfoLine label={text.file} value={fileName} />
      <InfoLine label={text.statusLabel} value={item.status} />
      {record.kind === "certificate" ? <InfoLine label={text.issuer} value={record.item.issuer} /> : null}
      <InfoLine label={text.expiry} value={expiry ? new Date(expiry).toLocaleDateString(localeForLanguage(language)) : text.noExpiry} />
      {record.kind === "document" && record.item.demo_content ? <View style={styles.preview}><Text style={styles.previewText}>{record.item.demo_content}</Text></View> : null}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  listCard: { flex: 0.9, minWidth: 300 },
  detailCard: { flex: 1.15, minWidth: 0 },
  row: { minHeight: 60, borderRadius: 13, borderWidth: 1, borderColor: workspaceColors.line, padding: 10, flexDirection: "row", alignItems: "center", gap: 10 },
  name: { color: workspaceColors.text, fontSize: 13, fontWeight: "800" },
  meta: { color: workspaceColors.muted, fontSize: 11, lineHeight: 16 },
  ...
  detailTitle: { color: workspaceColors.text, fontSize: 18, lineHeight: 24, fontWeight: "900" },
  bigStatus: { borderWidth: 1, borderRadius: 999, paddingHorizontal: 10, minHeight: 34, flexDirection: "row", alignItems: "center", gap: 6 },
  divider: { height: 1, backgroundColor: workspaceColors.line, marginVertical: 16 },
  issue: { borderWidth: 1, borderColor: workspaceColors.line, borderRadius: 12, padding: 11, flexDirection: "row", alignItems: "center", gap: 10 },
  issueInteractive: { backgroundColor: "rgba(255,255,255,0.018)" },
  issueTitle: { color: workspaceColors.textSoft, fontSize: 12, lineHeight: 17, fontWeight: "800" },
  recordLink: { flexDirection: "row", alignItems: "center", gap: 3 },
  recordLinkText: { fontSize: 10, fontWeight: "800" },
  okBox: { borderWidth: 1, borderColor: `${workspaceColors.green}44`, backgroundColor: `${workspaceColors.green}0D`, borderRadius: 12, padding: 12, flexDirection: "row", alignItems: "center", gap: 8 },
  requirementsHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12 },
  requirements: { flexDirection: "row", flexWrap: "wrap", gap: 7, marginTop: 12 },
  req: { borderRadius: 999, borderWidth: 1, borderColor: workspaceColors.line, paddingHorizontal: 9, paddingVertical: 6, flexDirection: "row", alignItems: "center", gap: 5 },
  reqText: { color: workspaceColors.textSoft, fontSize: 10, fontWeight: "700" },
  center: { flex: 1, alignItems: "center", justifyContent: "center", gap: 12, padding: 24 },
  muted: { color: workspaceColors.muted, fontSize: 12, textAlign: "center" },
  retry: { borderWidth: 1, borderRadius: 10, paddingHorizontal: 13, paddingVertical: 8 },
  editorBody: { gap: 14 },
  sourceBody: { gap: 10 },
  sourceTitle: { color: workspaceColors.text, fontSize: 17, fontWeight: "900", marginBottom: 4 },
  infoLine: { flexDirection: "row", justifyContent: "space-between", gap: 16, paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: workspaceColors.line },
  infoLabel: { color: workspaceColors.muted, fontSize: 11, fontWeight: "700" },
  infoValue: { color: workspaceColors.textSoft, fontSize: 11, fontWeight: "700", textAlign: "right", flex: 1 },
  preview: { marginTop: 6, borderWidth: 1, borderColor: workspaceColors.line, borderRadius: 12, padding: 12, backgroundColor: "rgba(255,255,255,0.018)" },
  previewText: { color: workspaceColors.textSoft, fontSize: 12, lineHeight: 19 },
});
